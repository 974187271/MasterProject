...
df_update = pd.read_csv(uploaded_file_path)
df_update = df_update.drop(columns=['Unnamed: 0'])
count = 0
for index, row in address_df.iterrows():
    ...
    df_modified['Address'] = address
    df_modified['Flag'] = flag
    df_modified['Index'] = count
    df_modified = df_modified.drop(columns=['Unnamed: 0'])

    df_update = pd.concat([df_update,df_modified], ignore_index=True)
    df_update.to_csv(updated_df_path,index=True)
